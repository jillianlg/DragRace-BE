...
async function scrapeDragRace() {
  ...
  for(const tds of trsTds) {
    if(tds.length === 5) {
      ...
      seasons[seasons.length - 1][1].contestants.push({
        name: await tds[1].evaluate(node => node.textContent.trim()),
        link: await tds[1].evaluate(node => node.querySelector('a')?.href),
      ...
      seasons[seasons.length - 1][1].contestants.push({
        name: await tds[0].evaluate(node => node.textContent.trim()),
        link: await tds[0].evaluate(node => node.querySelector('a')?.href),
  ...
  fs.writeFileSync('./scrapeDragRace.json', JSON.stringify(Object.fromEntries(seasons), null, 2), (err) => {
